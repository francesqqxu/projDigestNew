...
$(function(){
	...
	function projDigest_fm_load_edit(row){
		...
		$('#fieldCategory').combobox({
			...
			onLoadSuccess:function(){
				...
				if(null==field || ""==field){
					$(this).combobox('setText',fields[0].field);
					$(this).combobox('setValue',fields[0].id);
					return;
				}
		...
		$('#applicationCategory').combobox({
			...
			onLoadSuccess:function(){
				...
				if(null==app || ""==app){
					$(this).combobox('setText',apps[0].field);
					$(this).combobox('setValue',apps[0].id);
					return;
				}
	      ...
	      $.ajax({
			  ...
			  success: function(data,response,status){
					if(data.success==true){
						...
						$('#industryCategory').combobox('reload','industry/get_industryCategory');
	...
	$('#btnAddField').click(function(){
	      ...
	      if(null==industryId || ""==industryId){
	    	  $.messager.alert("系统提示","请先选择对应的行业类别");
	    	  return;
	      }
	      ...
	      $.ajax({
			  ...
			  success: function(data,response,status){
					if(data.success==true){
						$.messager.alert("系统提示","保存成功！");
						industryId = $('#industryCategory').combobox('getValue');
						$('#fieldCategory').combobox('reload','field/get_fieldByIndustryId?id=' +  industryId);
						$('#fieldCategory').combobox({
							valueField: 'id',
							textField: 'field'
						});
	...
	$('#btnAddApp').click(function(){
	      ...
	      if(null==fieldId || ""==fieldId){
	    	  $.messager.alert("系统提示","请先选择对应的领域类别");
	    	  return;
	      }
	      ...
	      $.ajax({
			  ...
			  success: function(data,response,status){
					if(data.success==true){
						$.messager.alert("系统提示: "," 保存成功！");
						$('#applicationCategory').combobox('reload','app/get_appByFieldId?id=' +fieldId);
						$('#applicationCategory').combobox({
							valueField: 'id',
							textField: 'application'
						});
	...
	*/
	...
	$('#projDigest_add_submit').click(function(){
		...
		if( $('#projDigest_fm').form('validate')){
			$.ajax({
				url: url,
				type: 'POST',
				timeout: 100000,
